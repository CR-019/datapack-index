...
后端的工作应该做到：在反馈前端对话框的操作后，将当前的游戏状态按照要求存储到指定的storage里，以方便前端对话框读取和展示。在推箱子这个小游戏下，我们可以把固定的地图存储为一个二维的列表，玩家可移动区域![](path.png)记为0，障碍物区域![](obstacle.png)记为1；然后将可变的玩家![](player.png)、箱子![](box.png)、目标点![](target.png)分别用2元列表存储，以供对话框部分处理。
