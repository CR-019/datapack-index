        ...
        <div class="meta-section">
          ...
          <div class="author-container">
            ...
            <div class="social-links">
              <a v-for="(link, index) in socialLinks" :key="index" :href="link.url" class="link" target="_blank">
                {{ link.name }}
              </a>
            </div>
          </div> <!-- /主作者信息块 -->
          ...
          <!-- 额外作者信息块 -->
          <template v-if="extraAuthors && extraAuthors.length">
            <div v-for="(extraAuthor, index) in extraAuthors" :key="index" class="author-container">
              <div class="author-info">
                <!-- 为额外作者的头像和名字添加链接 -->
                <a
                  v-if="extraAuthor.socialLinks && extraAuthor.socialLinks.length > 0"
                  :href="extraAuthor.socialLinks[0].url"
                  class="author-link"
                  target="_blank"
                >
                  <img :src="extraAuthor.avatarUrl" :alt="extraAuthor.authorName" class="avatar" />
                  <div class="author-details">
                    <h4 class="extra-author-name">{{ extraAuthor.authorName }}</h4>
                  </div>
                </a>
                <!-- 如果额外作者没有社交链接，则不包裹链接 -->
                <template v-else>
              ...
              <div class="social-links">
                <a
                  v-for="(link, linkIndex) in extraAuthor.socialLinks"
                  :key="linkIndex"
                  :href="link.url"
                  class="link"
                  target="_blank"
...
.author-container {
  display: flex;
  flex-direction: column;
  align-items: flex-start; /* 宽屏下左对齐 */
  gap: 0.5rem; /* 控制头像名字和社交链接之间的间距 */
}
...
@media (max-width: 768px) {
  ...
  .meta-section {
    ...
    align-items: flex-start; /* 顶部对齐作者块 */
  ...
  .author-container {
    display: flex;
    flex-direction: column;
    align-items: center; /* 窄屏下，作者块内部内容居中 */
    text-align: center;
    gap: 0.5rem;
    flex: 0 1 auto; /* 宽度由内容决定，允许收缩 */
    min-width: min-content; /* 防止被过度压缩 */
    /* 移除宽屏下的分隔线和间距 */
  ...
  .author-container:not(:last-child)::after {
      ...
      position: absolute;
      top: 50%;
      right: -1rem; /* 定位在右边距的中间 */
      transform: translateY(-50%);
      height: 4rem; /* 线条高度 */
      width: 1px; /* 线条宽度 */
      background-color: rgba(0, 0, 0, 0.2); /* 线条颜色 */
  }
