...
首先，您需要先获取Minecraft游戏。虽然1.13版本就添加了数据包，但我们强烈推荐您在刚上手时直接使用当前的正式版最新版。不出意外的话，本文也会按照当前的最高版本26.3维护。
...
记分板可以存储任意数量的32位带符号`int`。`scoreboard`命令可以对两个记分板上的数据做基础运算，包括赋值，max，min，交换，加减乘除模。所有的运算都只有两个运算目标；这意味着单独一条命令永远都是`x += y`的形式，而想要得到`x = y + z`的形式则需要多条命令。

26.3版本加入的数值提供器和`compute`支持一些更复杂的计算，例如平均值，乘方，开方，三角函数等。
...
存储在Storage中的数值可以使用26.3版本加入的数值提供器和`compute`进行计算。

Storage能够以复杂的结构存储复杂数据。此外，Minecraft中还有不少对象（即实体与方块实体）有属于自己的数据，用和Storage一样的格式存储。这些数据可以被复制进Storage中操作或存储……因此Storage实际上极为重要。
