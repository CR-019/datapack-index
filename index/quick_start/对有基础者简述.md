...
[Minecraft Wiki](zh.minecraft.wiki)上包含了大量的重要参考信息，它们在绝大多数情况下也都适用于最新版。

同时，虽然数据包能在大部分情况下和模组兼容，但刚上手时我们强烈建议您使用原版游戏以避免可能由模组带来的问题。

### VS Code

虽然有不少正在开发中或是测试中的，使用其他文本编辑器或IDE的，提供数据包开发环境的项目，目前的主流方案仍然是使用VS Code和以下插件：
...
```
其中`description`为数据包的显示描述信息，可以任意修改。`min_format`和`max_format`分别为本数据包兼容的最高和最低的数据版本。`pack.mcmeta`的详细结构可以通过阅读数据包元数据页面获取，但在本入门简述中您无需修改它。
...
本篇章将介绍`mcfunction`的语言逻辑，以为读者提供一些基础概念和参考。Wiki的[命令](zh.minecraft.wiki/w/命令)界面有对所有命令的详细介绍，应该作为您的主要参考。此外，香草图书馆的[参考](/index/参考)页面包含了针对不同命令的详细教程。
...
管理记分板的命令为`scoreboard`。有关`scoreboard`命令的详细语法请见wiki，或查阅图书馆内教程。
...
Storage使用命名空间格式，同样不需要声明就可以直接使用。它使用`data`命令操作，可以进行数据之间的读取，覆写，复制，字典合并，列表插入，删除，以及简单的字符串转换与切割。详见Wiki与图书馆内教程。
...
除非一个命令特别地允许，否则你没办法把变量传给命令，只能硬编码。1.20.2后出现的宏行以一定的性能代价解决了这个问题。它允许你在运行时使用宏“组装”出一条命令，游戏实时加载执行。这里不详细探讨，可阅读Wiki的[函数](zh.minecraft.wiki/w/Java版函数)界面。
...
Wiki的[命令](zh.minecraft.wiki/w/命令)界面有对所有命令的详细介绍。此外，香草图书馆的[参考](/index/参考)页面包含了针对不同命令的详细教程。香草图书馆也有大量页面专精于命令系统的各个方面。
...
Wiki的[数据包](zh.minecraft.wiki/w/数据包)界面会告诉你所有可以注册的东西。
