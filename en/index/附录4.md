...
## Block that does not block the entity
> by ethan
...
## Applicable item tooltip vertex id from 1.17 to 1.21
> by Xuanyu1725
...
1. Do not add entities unless necessary
2. It is best to use scoreboard when calculations are involved.
3. store result is best used only when converting data types
4. Try to only use scoreboard when judging values.
5. In judgments that do not involve nbt, try to use predicate (there are no complete test results for this one)
6. Use of selector Try to use execute to change the executor, and then use @s in the function instead of calling the selector multiple times.
7. You may need to consider the performance loss of function calls
...
10. For index type design, consider scoreboard tree
...
12. Do not attempt to complete operations in 1GT if the actual calculation amount is greater than 1GT.
13. If some operations involving a large amount of calculation are involved, they can be completed using player ticks that do not affect the experience, such as init, before switching dimensions, etc.