...
- world
  ...
  - **Hub boxes, eye height and passenger height changes** for **baby bodies** of armadillos, hoglins and zombie hoglins, axolotls, camels, chickens, cows and mooshrooms, dolphins, cats and ocelots, foxes, bees, goats, merry ghasts, pandas, pigs, polar bears, sheep, squids, wolves, striders, piglins, all zombie variants, and nautilus. Please [subject to Wiki](https://zh.minecraft.wiki/w/Java版26.2#更改).
- particles
  - **Now, instead of discarding new particles when there are too many particles, the game will randomly select particles to render**.
-command
...
- Game content
  - **Respawning in spectator mode in a hardcore world no longer turns off the game rules`spectators_generate_chunks`**.
-tag
  - **Rename blocktag**`#concrete_powder` -> `#concrete_powders`.
  - **blocktag`#mineable/pickaxe`and`​#happy_ghast_avoids`The dripstone cone in is replaced by`#speleothems`**.
  - **entity type tag`#cannot_be_pushed_onto_boats`Added sulfur cube monster**.
- NBT
  - **mob`HurtByTimestamp`Change to`ticks_since_last_hurt_by_mob`. Record the time since last injury instead of the timestamp**.
...
-predicate
  - **entitypredicate`entity_properties`All fields in are expanded into data component-like forms**.
    - This means that the top-level field names are now in the form namespaceID (`effects` -> `minecraft:effects`, minecraft can be omitted).
...
- world generation
  - Configured features
    - **The following features have been destructively renamed and/or modified. Please refer to Wiki**:
      ...
      - `large_dripstone`
      - `tree`
      - `multiface_growth`- Processor list
...
- itemmodel mapping
  - Removed special model types`bed`，`standing_sign`,and`hanging_sign`.
- Texture
...
- command
  ...
  - **Slots removed`villager.*`. The item slots of villagers and piglins can now be used`mob.inventory.*`Come visit. **
-recipe
...
- world
  - **The collision boxes of some baby mobs have been changed. **Includes cows, sheep, ocelots, mooshrooms, squids, glowing squids, zombies, husks, drowned zombies, piglins, zombie piglins, villagers, zombie villagers, cats, chickens, horses, wolves, pigs, rabbits, axolotls, and skeleton horses.
  ...
  - **Signboards and flags`rotation`The default value of the block attribute changes from`0`Change to`8`. **
- Data format
...
- Configured features
  - **The following feature types have been renamed**:
    -`forest_rock` -> `block_blob`, `ice_spike` -> `spike`- **Removed`flower`, `flower_no_bonemeal`, and`random_patch`Feature type. **
...
- Sound effect format
  - **Wolf sound variant definition format moves all previous fields to`adult_sounds`within and joined`baby_sounds`Sound effect representing a young wolf**.
...
- itemmodel mapping
  - **Some model types now have new`transformation`Fields, similar to display entities, can transform the model. Therefore, some special types of transformations are no longer hard-coded, but need to be specified in the itemmodel mapping. **
...
#### data pack：
-command
  - **/WorldBorder's time parameter now uses game ticks as the unit by default. **
...
-tag
  ...
  - **The following mob biotags have been replaced by new environmental attributes and therefore removed:**
...
- world
  ...
  - **Removed`spawnChunkRadius`game rule because the concept of a spawn-point chunk no longer exists. **
  - **World boundaries for each world are now independent. **
...
- Data components
  ...
  - **`profile`Components now have both static and dynamic behaviors: **
    - **static**: when this component has`properties`, having at the same time`name`and`id`field or neither of the above two fields.
      - The file will always be the same as when it was generated. The displayed skin is frozen when the component is created.
    - **Dynamics**:`name`and`id`There is only one field.
...
#### resource pack:
  - shader:
    ...
    - **Vertex shaders used for post-processing effects, luminance map generation and full-screen blit transfer are no longer passed`Position`attribute, and must pass`gl_VertexID`Assign vertex coordinate. **
...
- **Added required fields`min_format`and`max_format`Specifies supported versions. **
  - The format is`[major version number, minor version number]`. If only one integer is filled in, it is equivalent to`[major version number, 0]`。
...
-tag
  - **Rename the following blocktag:**
    -`#plays_ambient_desert_block_sounds` → `#triggers_ambient_desert_sand_block_sounds`- Data components
...
- model
  - **Now in itemmodel mapping, if the size of the item in the GUI is larger than one grid, you need to add`"oversized_in_gui": true`, otherwise it will be cut off at the edge of the grid. **
- Sound:
...
- Miscellaneous
  - **player head item model no longer changes from`profile`Read the texture from the data component. Added new player_head model class for rendering`profile`texture. **

### **1.21.5**
#### data pack：
- SNBT format:
  - Integers can now be`0`start.
  - Scientific notation is no longer allowed (e.g.`1e1000`) represents a number.
- NBT:
  ...
  - **item display frame, fluorescent item display frame, and knotted ones`TileX`，`TileY`,and`TileZ`was merged into`block_pos`. **
...
-tag
  - Rename the following blocktags:
    -`#dead_bush_may_place_on` → `#dry_vegetation_may_place_on`- Text component
...
-predicate
  - **entitypredicate is added`components`Used to match entity components. the following`type_specific`The entity sub-predicate has been removed and moved in`components`middle**:
...
- world generation
  - **`patch_pumpkin`and`patch_sugar_cane`The order of generation is reversed. **
- Miscellaneous
...
- **namespaceID changes for the following textures**:
    ...
    - `.../cow` → `.../temperate_cow`
    - `.../pig` → `.../temperate_pig`
    - `entity/chicken.png` → `entity/chicken/temperate_chicken.png`- **Size changes for the following textures**:
    -`temperate_cow`
    - `temperate_pig`
    - `red_mooshroom`
    - `brown_mooshroom`- **The following textures have been separated into separate files from their original texture files**:
    ...
    - `entity/horse/horse_zombie.png` → `entity/equipment/zombie_horse_saddle/saddle.png`- model
...
-shader
  ...
  - ** in each`uniform`，`type`It is now required. accept any`int`，`ivec3`，`float`，`vec2`，`vec3`，`vec4`,and`matrix4`。**
...
- data pack components:
  ...
  - loot table, predicate, item modifier:
    - **Join`all_of`condition,`alternative`The condition is renamed to`any_of`**
  - tag：
...
*tag
  * Rename blocktag:`snow_step_sounds` → `inside_snow_step_sounds`* block
