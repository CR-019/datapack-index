---
title: 'Use dialog to make 2D mini games'
---
...
## Introduction
In 25w20a, mojang introduced the definition of dialog, allowing the player to customize a dialog similar to the pause interface. Therefore, I thought about whether I could use dialog to make 2D games, such as pushing boxes, and did some practice. Below are the results of my research.

::: tip
In this article, we will not stick to the syntax of the dialog, but focus on explaining the implementation ideas and technical details that need attention. Readers who want to start with dialog from scratch can go to the wiki to check it out, or search for other tutorials.
:::

## Backend preparation
Let’s take the box pushing game I made as an example. The game logic processing part is not the focus of this article and will be skipped here;
The back-end work should be as follows: after feeding back the operation of the front-end dialog, store the current game state in the designated storage as required to facilitate the reading and display of the front-end dialog. In the small game of pushing boxes, we can store the fixed map as a two-dimensional list, with the player's movable area recorded as 0 and the obstacle area recorded as 1; then the variable player, box, and target point are stored in 2-element lists respectively for processing in the dialog part.
...
The game interface needs to dynamically display the current positions of the player and the box, and needs to dynamically read data, so pre-registered dialogs cannot be used. Fortunately, mj has added support for inline dialog in the dialog command, which means that we directly connect a dialog parameter that meets the requirements after the command, or we can directly generate a dialog. Combined with the function macro, we can read the current status display of the game in real time.
...
Next, we pre-insert the unchanged part of the dialog into the storage:
...
At the same time, the default button behavior will close the dialog, which will cause the mouse cursor to reset to the center of the screen and cause a splash screen, which greatly affects the experience, so we need to modify the default button behavior to none and do not perform unnecessary operations.
noticed that the current body part, which is the part that displays text, is empty, because we need to dynamically read the map information mentioned above and display the terrain and player position.
...
is a two-dimensional list, so it uses two levels of traversal. First, add a line to the list, set the font and other formats to be used, and then in the second level of traversal, obtain the information of each coordinate and display the corresponding font.
...
The vertical line spacing cannot be changed currently, so the height of the grid unit is set to twice the line spacing, so that two line breaks can be used to make the grid seamlessly spliced ​​vertically.

Next we read the coordinates of players, boxes, etc. and display them on the map:
...
For simple processing, character images such as player and box include the grid below, so we only need to read the positions of these icons, and then find the corresponding coordinates and replace them directly.
...
## Buttons and routing
solve the display problem, let’s take a look at the operation problem.
...
Due to the limitations of mj, executing instructions with permission requires a second confirmation (a pop-up window will pop up), which is undoubtedly unacceptable in small games that require frequent operations. Therefore, we need to use the trigger command with permission level 0 to trigger indirectly.

>  Players who have entered command in newer versions may not know what trigger is. In fact, I didn’t know anything about it before I made this little game. Simply put, trigger is a special type of scoreboard criterion. After being activated, its value can be modified using the /trigger command. The data pack can then poll for changes in its value to trigger some instructions just like it detects other scoreboard interfaces.
...
is: detect the scoreboard score and perform the corresponding operation function, then reset the scoreboard, and finally refresh the dialog.
...
It is indeed a very interesting practice to use static dialogs to create dynamic mini-games. Many of the poisonous points that destroyed the experience have been basically solved in 1.21.6 pre1, and it is now in a quite usable state.
