...
1. Mark the model (surface) by texture, that is, the idea of storing more data in the color channel
...
![alt text](../../../../../feature/archive/202602/2/2026-01-18%2016-14-12.gif)

## 1. Mark the model (surface) by texture

In this example, we only modify a specific model to display the code rain effect on its surface. In order to achieve this, we need to store more data in the color channel so that these models can be recognized in the fragment shader.
...
> Although solid type rendering objects cannot output transparency channels, they can still have opacity.

When detecting, we use`textureLod()`function to read the color channel data and determine whether the opacity is equal to$254/255$to identify the model that needs to display the code rain effect. For instructions on this function, please refer to previous tutorials [Core shader workflow (Part 2)](/en/feature/archive/202511/2/content) or official documentation.

```glsl
bool roughly_equals(float a, float b) { //Due to the accuracy problem of floating point numbers, it is not suitable to directly use the equal sign to judge
    return abs(a - b) < 0.000001;
}

vec4 color = textureLod(Sampler0, texCoord0, 0.0); //Set the Lod level to 0 to avoid errors caused by Mipmap
if (roughly_equals(color.a, 254.0 / 255.0)) {
    //Here is the implementation of the code rain effect
}
```
## 2. Use of coordinate sampling from the screen

We want the code rain effect to be sampled directly at screen coordinates like the end portal, so we need to use`projection_from_position()`function gets the screen coordinate.

```glsl
//This function is defined in projection.glsl
...
## 3. Calculate the mapping relationship from submap coordinates to sprite map coordinates

Since Minecraft's baked models are composed of voxels, Minecraft renders each quadrilateral face (2 triangles, sharing 4 vertices) in sequence. So, GLSL’s built-in variables`gl_VertexID % 4`The result is that it cycles from 0 to 3 as each quad face is rendered.
...
```
Now we hope to find the mapping relationship from sub-picture coordinates to sprite map coordinates. Assuming there is no rotation or inversion between the sub-picture area and the sprite map area, then we have the following relationship:$$\begin{pmatrix}\text{texCoord0.x} \\ \text{texCoord0.y}\end{pmatrix} = \underbrace{\begin{pmatrix}k_1 \\ k_2\end{pmatrix} \odot \begin{pmatrix}\text{NormalizedUV.x} \\ \text{NormalizedUV.y}\end{pmatrix}}_{\text{component-wise multiplication}}  + \begin{pmatrix}b_1 \\ b_2\end{pmatrix}$$Then we need to find the vector$\begin{pmatrix}k_1 \\ k_2\end{pmatrix},\begin{pmatrix}b_1 \\ b_2\end{pmatrix}$Obviously, under the pipeline limitations of Minecraft, we cannot obtain enough information in vsh. Therefore, we need to use the partial derivative function in fsh to solve, that is, use the rate of change of the known variable on the screen coordinate to solve the unknown quantity in the above equation.$$\begin{pmatrix}k_1 \\ k_2\end{pmatrix} = \frac{\mathrm{d}\text{texCoord0}}{\mathrm{d}\text{NormalizedUV}}$$In fact,$\text{texCoord0}$and$\text{NormalizedUV}$It’s about screen coordinate$(x,y)$function, then we can write two independent component equations:

To avoid notational confusion, we will$\text{texCoord0}$and$\text{NormalizedUV}$respectively recorded as$T$and$S$. The two components are$T_1,T_2$and$S_1,S_2$. Then the above formula can be written as.$$T_1(x,y) = k_1 \cdot S_1(x,y) + b_1 \\
T_2(x,y) = k_2 \cdot S_2(x,y) + b_2$$Correct the above two equations respectively$x$Finding the partial derivative, we get:$$\frac{\partial T_1}{\partial x} = k_1 \cdot \frac{\partial S_1}{\partial x}$$

$$\frac{\partial T_2}{\partial x} = k_2 \cdot \frac{\partial S_2}{\partial x}$$Similarly, for the above two equations, respectively$y$Finding the partial derivative, we get:$$\frac{\partial T_1}{\partial y} = k_1 \cdot \frac{\partial S_1}{\partial y}$$

$$\frac{\partial T_2}{\partial y} = k_2 \cdot \frac{\partial S_2}{\partial y}$$You can choose either of the two expression methods. Here we choose$x$way to find partial derivatives. Then we can solve:$$k_1 = \frac{\partial T_1 / \partial x}{\partial S_1 / \partial x}$$

$$k_2 = \frac{\partial T_2 / \partial x}{\partial S_2 / \partial x}$$Since the partial derivative function and division in GLSL are component-wise operations, we can directly write:$$\begin{pmatrix}k_1 \\ k_2\end{pmatrix} = \frac{\partial T / \partial x}{\partial S / \partial x}$$> Note: Operator$\frac{\partial}{\partial x}$i.e. function`dFdx()`, represents the screen coordinate$x$Find the partial derivative. In the same way, we have$\frac{\partial}{\partial y}$i.e. function`dFdy()`.

Next we find the vector$\begin{pmatrix}b_1 \\ b_2\end{pmatrix}$. From the above component equations, we can solve:$$b_1 = T_1 - k_1 \cdot S_1$$

$$b_2 = T_2 - k_2 \cdot S_2$$Therefore we can directly write:$$\begin{pmatrix}b_1 \\ b_2\end{pmatrix} = T - \begin{pmatrix}k_1 \\ k_2\end{pmatrix} \odot S$$The following is the code implementation:

```
glsl
vec2 k = dFdx(texCoord0) / dFdx(NormalizedUV);
vec2 b = texCoord0 - k * NormalizedUV;
vec2 SpriteUV = k * SpriteNormalizedUV + b; //This line is used in the final sampling. In the drawing code, we assume that sampling is done within the subgraph.
```
> Note that the correctness of the above formula depends on there being no rotation or inversion between the submap area and the sprite map area. If there is rotation or inversion, the above method will not be able to calculate the mapping relationship correctly. Therefore, the code for setting subgraph coordinates in vsh must ensure a specific order, **and Mojang has a precedent of modifying the vertex order between different versions**, so please be sure to test and verify it in actual use.
...
In the main drawing code, CELLS defines the number of columns and rows of the code rain.`pix`The calculation result is the offset coordinate of the current fragment within a cell,`cell`The calculation result is the coordinate of the cell where the current fragment is located.`pix *= CELLS * vec2(0.8, 1.0) + vec2(0.1, 0.0);`What this line of code does is`pix`Zoom and pan so that the characters of Code Rain have a certain margin within the cell, thereby preventing the characters from sticking to the edge of the cell. ultimately a$[0.1,0.9)$numbers between.
...
out vec2 texCoord0;
out vec2 NormalizedUV; //An output variable is added here
...
void main() {
    ...
    texCoord0 = UV0;
    if (gl_VertexID % 4 == 0) { //Calculate subgraph coordinates by vertex ID, pay attention to the order
        NormalizedUV = vec2(0.0, 1.0);
    } else if (gl_VertexID % 4 == 1) {
        NormalizedUV = vec2(0.0, 0.0);
    } else if (gl_VertexID % 4 == 2) {
...
in vec4 vertexColor;
in vec2 texCoord0;
in vec2 NormalizedUV; //An input variable is added here

out vec4 fragColor;

//The code here is the sampling function added to the original rendering of the new version. It may seem unfamiliar. We do not need to understand its implementation details in depth.
...
#define CELLS vec2(128.0,60.0) // This item configures the number of columns and rows of code rain
#define FALLERS 4.0 // Determines the number of falling raindrops simulated simultaneously on the same column. The larger the value, the more raindrops appear on the same column at the same time and the denser they appear.
#define FALLERHEIGHT 16.0 // Controls the visual length of a single raindrop and the period of its falling cycle. The larger the value, the longer the raindrop and the longer the time between resets.

vec2 rand(vec2 uv) { //Pseudo-random number generation function based on trigonometric function
    vec2 r = floor(abs(mod(cos(
        uv * 652.6345 + uv.yx * 534.375 +
        GameTime * 1200 * 0.0000005 * dot(uv, vec2(0.364, 0.934))),
     0.001)) * 16000.0);
...
float fallerSpeed(float col, float faller) { //Velocity generation function based on trigonometric function
    return mod(cos(col * 363.435 + faller * 234.323), 0.1) * 1.0 + 0.3;
}
