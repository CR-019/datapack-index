...
The technology used in this work includes many new features updated in the past two years: dialog, itemmodel mapping, function macros, and many technologies derived from them. Next, I will conduct a technical analysis of the project from three perspectives: chessboard model, chess game interface, and back-end system.
...
After getting these models, we can use "assemble the model" in itemmodel mapping (`composite`Type mapping) combines these models.
...
For example`0`item representative`(1,1)`,`14`Item representation`(2,7)`. The value of the item represents the chess piece on this grid,`empty`It means there are no chess pieces, and when there are chess pieces, it means`(piece)_(faction)`Indicates the type of chess piece. \
...
```mcfunction
#Calculate chess piece coordinates
...
As a dialog that displays the chess game in real time, we naturally need to use the inline macro dialog to achieve dynamics. For the specific implementation method, see [the chapter from the previous issue of "Picking Dust"](/en/resources/dust/8/对话框小游戏.md), which will not be described again here. \
...
> The effect of direct stitching is as follows:
...
::: warning Some tips on font textures
