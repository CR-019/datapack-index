...
- [Xiaodou’s math library **v3.1** (main body)](https://github.com/xiaodou8593/math3.1)&nbsp;&nbsp; `~1.20.4-1.21~`- The main functions of this math library depend on the unit library and need to be installed together with the main body:
...
### Control a single entity not to be affected by group hatred
...
1. Have a very low`follow_range`A mob will not join its hostile group and will not anger its group when attacked.  
...
Limitations: Behavior is not always perfect. Sometimes when you attack individual mobs directly, 1 or 2 mobs will still be enraged. This usually happens when group mobs are very close to individual mobs when attacked, so works best in areas with lower mob density. Also, this package is not optimized for performance, but I think it's good enough to share.https://github.com/oligomc/mccrowdcontrol:::warning Editor's Note
