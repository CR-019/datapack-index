...
![file](../../../../../feature/archive/202604/2/file.jpg)

- `packsquash.exe`:PackSquash executable file
- resource pack directory: decompressed resource pack, placed in a directory,`pack.mcmeta`It should be in the root directory of this directory
  - ![resource pack directory](../../../../../feature/archive/202604/2/pack.jpg)
- Configuration file: a format of`toml`text file, the name is not limited, but here I named it`config.toml`Theoretically they can be placed anywhere, but for convenience, they are placed in the same directory here

## Configuration
open`config.toml`, enter the following:

```toml
pack_directory = 'path to the resource pack directory'
```
Will`path to the resource pack directory`Replace with the actual path to your resource pack directory

As for how to obtain the resource pack path, a simpler solution is to right-click your directory and select`Copy as path`![copy path](../../../../../feature/archive/202604/2/copy_path.jpg)

If it is another version of Windows, it may not be available.`Copy as path`options, you can choose`Properties`, found in the pop-up window`Location`, copy its contents
...
Open the terminal in the working directory. Generally, you can right-click in a blank space of the working directory and select`Open in Terminal`, if this option is not available, you can copy the path of the working directory, start the terminal separately, and enter`cd "path"`Enter working directory
...
If you are too lazy to bother, the author wrote a set based on compression effects. Generally speaking, it is reliable. If there are problems with use, please check the documentation and modify it yourself.
