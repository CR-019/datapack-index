...
Doing this will generate a number of parabolas, the number of which is determined by the parameter`n`control\
parameters`t`Control the existence time of each parabola
...
When the parabola's existing time is reduced to 0, the parameters will be executed`cmdv`function event in, call`sklibs:skfirework/fx/firework`function generates fireworks, where`args`is the parameter of function
...
## APIfunction`sklibs:skfirework/fx/launch`Fireworks start function, used to create the lift-off stage of fireworks
...
</div>`sklibs:skfirework/fx/spread`\
Fireworks diffusion function
<div class="nbttree">

<node type="compound" name="config" />function parameters
- <node type="int" name="n" />Quantity
- <node type="homolist" name="yaw" />Yaw angle random interval, default [-180, 180]
...
</div>`sklibs:skfirework/fx/firework`\
...
- <node type="homolist" name="colors" />random colors
  - <node type="compound" name="(list element)" :colon="false" />
    **tags common to random color items**
    ...
    - <node type="int" name="trail" /> Whether to display trails
