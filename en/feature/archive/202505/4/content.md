 ...
 * **Semantic Highlighting**: Provides semantic coloring of command parameters to enhance the readability and maintainability of the code.
 * **Auto-completion**: The intelligent completion function covers simple commands to complex NBTtags, greatly reducing input errors.
...
## NBT Autocomplete and other intra-office auxiliary tools
...
`data get`Command can be used to view data of entity, block or item. A player who has just started learning Minecraft commands and data packs, or a player who has a certain foundation but wants to learn more about data operations, may try to create custom items, adjust entity behavior, or make more complex data packs. But when you encounter the problem that data types are not applied correctly, you need tools to obtain and inspect the data.

For novice players,`data get`Command can help the player see the essence through the phenomenon. Let's combine it with Wiki and take an armor stand as an example. The picture below shows an armor stand:

![Armor Stand](../../../../../feature/archive/202505/4/img/image-5.png)

At first glance, it looks like a floating character, written "1639". However this is actually an entity, let's`/data get entity`Take a look. Note that /data get can only obtain the data of a single entity, so the selector must be restricted during operation, for example, to obtain a single nearest armor stand:
...
Or for the entity pointed by the mouse pointer, you can directly use the Tab key to refer to the entity's unique UUID. For details, see [Universal Unique Identifier - Chinese Minecraft Wiki](https://zh.minecraft.wiki/w/?curid=56543). It can also be used for the item pointed by the mouse pointer.`F3+I`Use the shortcut key to obtain entity data with one click.
...
*`CustomNameVisible`**Indicates whether the entity always renders the name, which means whether the player can see it**;
*`Invulnerable`** is also an important tag, which determines whether the entity can resist most damage. If true, the entity will only take damage from creative players and belong to **`#bypasses_invulnerability`**tag damage**.

It doesn’t look like an entity on the surface, but in fact`data get`To expose its true form, we can also start from this perspective when studying entity data: we can copy it! But it must be mentioned here that you must understand SNBT, which corresponds to [SNBT format - Chinese Minecraft Wiki](https://zh.minecraft.wiki/w/?curid=145849). An example here is`OnGround:1b`in`b`is a Boolean value,`1`represent`true`，`0`represent`false`.
...
```
For novice players, many other questions can also be used`data get`To achieve, and it is universal; for example, the enchantment problem mentioned above, we obtain the data of the enchantment item on hand, for example, we have a **Looting III** acacia boat on hand:
...
Newer versions of Minecraft (1.6+) have moved to a more modular file structure (.minecraft/versions folder). We find the corresponding version`.jar`Unzip the file (unzip it somewhere else, not where it is) or open the compressed package. Follow the path \data\minecraft and you can see the structure of the vanilla data pack.
...
Corresponds exactly to the Minecraft wikidata pack introduction. We can create by imitating the vanilla data pack format and data pack content. For example, let's create a **Accurate Collection II**; by modifying **vanillanamespace**`silk_touch.json`and`spawner.json`We have only briefly shown that Precision Gathering II can mine monster spawners, and the enchantment table will not enchant **Precision Gathering II**. This is an example of disassembling the vanilla.jar file to do data pack:
...
## Disassemble data pack instance
