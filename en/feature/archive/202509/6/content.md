...
## Introduction`/rotate`is a command used to change the orientation of an entity. When it is used to rotate an entity to face another entity, the syntax is:
...
```
`
Among the parameters`[&lt;facingAnchor&gt;]`The type is entity anchor, available parameters`feet`and`eyes`, respectively for the feet and eyes facing the target entity. If a beginner command needs to face the eyes of the nearest villager, due to human reading and writing habits, he is likely to write the command as:
...
The position of the eyes and feet in the horizontal direction is the same.$y$On the axis, the height of the entity's eyes is the difference between the heights of the eyes and feet.
...
The execution anchor point has fewer application scenarios than the execution position. It only affects the local coordinate, entity orientation and execution orientation, thus affecting`/tp`、`/rotate`and`/execute`Three commands`facing`The execution result of the subcommand. When the anchor point is the foot, the origin of the relative coordinate and local coordinate, the entity orientation, and the starting point of the execution orientation are all at the execution position (foot). When the anchor point is the eye, the origin of the relative coordinate is at the execution position, which is the foot; the origin of the local coordinate, the entity orientation and the starting point of the execution orientation are at the eye**.
...
```
`
This command sets the execution anchor point to the player's eye, and the execution position is still the center point of the bottom of the player's collision box. This position is relative coordinate`~ ~ ~`Use, so the stone will be placed in the block where the player's lower body is.
...
```
`
The execution anchor point is also set to the player's eye, but`^ ^ ^`Use the execution anchor point, so the stone will be placed in the block where the player's upper body is.
...
```
`
This command does not modify the execution anchor point, so the default anchor point is player (`@s`) of the feet. The starting point of the direction is the feet, and the end point is the villager specified by command.`eyes`, this is the player’s orientation:
![Explanation](../../../../../feature/archive/202509/6/解释.png)
