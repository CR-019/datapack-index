...
### Maintain the accuracy of entity rotation
...
### Instantly trigger the enchantment effect on the player
(1.21.6+)
...
Next, the game mode for the player is instantly switched to`spectator mode`Then switch back and you can trigger.
...
Then get the block every tick`LastOutput.text`value, use the string interception function to intercept by bits (1 9 is selected, of which 1 3 can be used to intercept hours, 4 6 to intercept minutes, and 7 9 to intercept seconds).  
(1.21.5+)
