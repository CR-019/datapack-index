...
### 3.3 Officially start implementing the lexical analyzer
...
- **Reference regular expression:**`[A-Za-z_][0-9A-Za-z_]*`
...
- **Reference regular expression:**`[0-9]+`
- Behavior: Multiply the stored number by 10 and add the new digit.
- Jump to:
...
- Behavior: No behavior. Comments are not submitted to the parser.
- Jump to:
  - Checked characters`*`Then enter the exit preparation node.
...
- Jump to:
  - If it is still a skippable character, jump to itself.
