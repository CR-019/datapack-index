...
3. Use recursion to subtract points from the temp scoreboards of all players. The one that reaches 0 first is the one with the lowest ranking, and it is stored in the scoreboard "ranked" for statistical ranking.
4. By analogy, when all playertemp scoreboards are reset to zero, the ranked scoreboard with the most points will be the highest ranked.
5. Make the score of virtual player.Ranked = the score of the highest-ranked player, add .Result to count the number of players participating in the ranking, and an .Opposite virtual player to perform reverse selection.
