...
- OBJ models are a universal 3D model format and Blockbench's Java models are Minecraft resource pack json.
...
- The texture path, parent model, UV, rotation and block state of the vanillablock model all need to be parsed correctly.
...
- Java Block/Item Models themselves have real limitations in size and performance.
...
- If you already have an OBJ exported from Mineways, it's more straightforward to do the OBJ import.
...
- Minecraft `.jar`or`.zip`

When blockstate is found, the plug-in will select the corresponding model based on the current block state; then parse the model json`elements`, convert each element into a Blockbench cube.
...
> The minecraft:water block cannot be imported at this time.
...
<img src="../../../../../feature/archive/202606/6/image2.png" width="50%">
</p>

After the import is successful and checked, you can use the Java Block/Item Model that comes with Blockbench to export the json file to the models/item directory of the resource pack. After defining the corresponding item model mapping file, the model can be used in the game.

<p align="center">
  

<img src="../../../../../feature/archive/202606/6/image4.png" width="75%">
</p>
...
## Usage process two: Directly import structure files
...
Select located in`存档根目录\generated\minecraft\structures\`nbt structure file under the pop-up window, focus on setting "texture source folder or Minecraft jar". It is recommended to directly select the Minecraft jar file corresponding to the version, or select an unpacked resource pack directory.
...
| Set up Java cullface | Write cullface information for the face and enable it as needed |
...
- Return the block ID and reason of the complete block
...
<img src="../../../../../feature/archive/202606/6/image5.png" width="50%">
</p>
...
<img src="../../../../../feature/archive/202606/6/image6.png" width="75%">
</p>

## About quantity and size restrictions
...
The default limit of the plug-in is 5000 cubes. After exceeding this number, Blockbench may still be able to be opened, but editing, selection, saving and verification will be slower. Please use your own judgment. If there are more than 10,000 cubes, it is usually recommended to import them separately, and then put multiple models together through item model mapping.

In addition, the Java Block/Item Model is not prepared for very large buildings. The common range of Blockbench's Java block model can be understood as about 3x3x3 block, that is, the coordinate is roughly`
- 16`arrive`32`between. Converted according to the default proportion of the plug-in, it is recommended that the final model should be controlled to approximately:
...
- Try to reduce the number of final cubes generated and merge continuous complete blocks.
...
The most common cause when importing OBJ is that the scale setting is too large. Minecraft Building OBJ Generally Recommended`OBJ 方块缩放 = 1`. If filled in to 16, each block will be enlarged 16 times, and the exported coordinates can easily exceed the common range of Java models.

### I don’t want to export a bunch of vanilla textures when saving
...
### Can it read skull textures, sign text, or flag patterns?
...
## What kind of workflow is suitable for

This plugin is best suited for the following situations:

- I want to quickly move Minecraft buildings into Blockbench for secondary editing.
