...
<div style="text-align:center">
  <div style="display:flex; justify-content:center; align-items:center; gap:8px;"><img src="../../../../../feature/archive/202606/0/image/接水管小游戏实例.png" alt="" style="zoom:20%;" /><img src="../../../../../feature/archive/202606/0/image/接水管小游戏实例1.png" alt="" style="zoom:20%;" /></div>
...
- There is only one tree in the picture, and all positions on the grid can be reached along the tree.
- The tree only contains pipes of the following shapes, **Cross-shaped pipes are not allowed to be generated**.
  ...
  - Straight pipe, with two directions: ━ and ┃.
  - L-shaped elbow, available in four shapes: ┏, ┓, ┗ and ┛.
  - T-shaped tube, available in four shapes: ┣, ┳, ┫, and ┻.
...
<div style="text-align:center">
<img src="../../../../../feature/archive/202606/0/image/接水管小游戏官网标注了出现环路的地方.png" alt="" style="zoom:30%;" /><p style="color: gray;">Figure 2: The official website of the water pipe game marks the places where loops occur</p>
</div>
...
<div style="text-align:center">
<img src="../../../../../feature/archive/202606/0/image/有向图在接水管旋转上的问题.png" alt="" style="zoom:20%;" /><p style="color: gray;">Figure 3: Problems with directed graphs in water pipe rotation</p>
</div>
...
<div style="text-align:center">
<img src="../../../../../feature/archive/202606/0/image/两种形态的环路.png" alt="" style="zoom:20%;" /><p style="color: gray;">Figure 4: Two forms of loops</p>
</div>
...
```
t = t_source  //t: node, t_source: root node
visit t
for n ∈ N do
  push (t, t_n) to list  //t_n: neighbor node of t
end for
while list is not empty do
  x = random(1, size of list)
  (t_c, t_cn) = list[x]  //t_cn: neighbor node of t_c
  if t_cn unvisited & t_c not t_shape then
    connect(t_c, t_cn)
    visit t_cn
    for n ∈ N do
      if t_cnn unvisited then  //t_cnn: neighbor node of t_cn
...
<img src="../../../../../feature/archive/202606/0/image/Prim算法流程图.png" alt="" style="zoom:20%;" /><p style="color: gray;">Figure 5: Prim algorithm flow chart</p>
</div>

### 2.2 Tarjan algorithm
To find a loop, you essentially need to find a cut point. Tarjan proposed an extremely classic local decision condition. For a parent node in the DFS tree$u$and its child nodes$v$,have:$$\text{low}(v) \ge \text{dfn}(u)$$in`dfn`is the traversal count of the node. Whenever an unvisited node is encountered,`dfn`will increase by 1, so it is always increasing, recording the order in which each node is traversed.`low`Refers to: slave node$A$Start and continue traversing forward. Under the premise of directly backtracking, follow other paths to traverse all the nodes you can reach.`dfn`The smallest, that is, the earliest traversed node`dfn`value.

The meaning of this formula is: subtree$v$All nodes in , no matter how we traverse forward through reverse edges, we can only traverse at most$u$, cannot be crossed$u$Reach a node further upstream. Therefore, once the$u$Delete, subtree$v$will be completely isolated. therefore,$u$It is a cutting point. In DFS back to$u$At that moment, the program only needs to check the child node's$\text{low}(v)$Whether this condition is met, it can be determined that the$u$The part of the subgraph that is the dividing line is an independent loop.
...
1. Initialize the disk surface and draw the grid according to the required length and width.
2. Generate the map using the randomized Prim algorithm.
3. Randomly shuffle the entire board.
4. Display the board to the player.
...
<img src="../../../../../feature/archive/202606/0/image/grid数据示意图.png" alt="" style="zoom:25%;" /><p style="color: gray;">Figure 6: Grid data diagram</p>
</div>
...
#Generate a grid of a specific size
function pipes:grid/

#Create a list of candidate edges
data modify storage pipes:prim alternative_side set value []

#Determine the starting grid to generate
...
#Create candidate edges for the starting grid
...
- <node type="byte_list" name="index"/>The direction of the candidate edge, there are 2 elements in total. The previous element is`-1b`When , it means that this side extends to the left; for`1b`When , this edge extends to the right. The next element is`-1b`When , it means that this side extends upward; it is`1b`, this edge extends downward, and there are actually only 4 valid values: left`[B;-1b,0b]`,superior`[B;0b,-1b]`,right`[B;1b,0b]`,Down`[B;0b,1b]`.
...
```mcfunction
#Extract from candidate edge list
...
#If the above checks pass, proceed to the following content
#Connect neighbor nodes to current node
...
#Mark neighbor nodes as visited
function pipes:prim/visit_neighbour with storage pipes:prim macro

#Include neighbor nodes into candidate edge list
...
### 3.4 Game process
The player's operations need to be monitored.`pipes.operation`It is a trigger-type scoring item. When the player rotates any node, the player's score on the scoring item is set to the corresponding node.`index`, so as to identify which node the player rotates. See Chapter 4 for the specific implementation method. The advancement used by the listening operation is as follows:
...
#Determine whether all water is filled
...
Below is the function that Tarjan's algorithm needs to run recursively. Since Minecraft's scoreboard and command storage are global variables, an explicit stack is required to coordinate recursive operations.
...
#Whether the neighbor node is not the parent node of the current node
execute if score #parent_tile_index pipes.var = #neighbour_tile_index pipes.var run return fail

#Determine the status of neighbor nodes
#none
...
#The low value of the current node and neighbor nodes takes the smaller value.
...
#If the low of the neighbor node is greater than or equal to the dfn of the current node
...
| | Corresponding characters | Ordinary water pipes | Water pipes for filling water | Water pipes that generate warnings | Water sources |
| --- | --- | --- | --- | --- | --- |
...
<img src="../../../../../feature/archive/202606/0/image/可视化排布方案.png" alt="" style="zoom:20%;" /><p style="color: gray;">Figure 8: Visual layout plan</p>
</div>
...
<img src="../../../../../feature/archive/202606/0/image/可视化过程的遍历顺序.png" alt="" style="zoom:25%;" /><p style="color: gray;">Figure 9: Traversal sequence of the visualization process</p>
</div>
...
<img src="../../../../../feature/archive/202606/0/image/最终成果.png" alt="" style="zoom:20%;" /><p style="color: gray;">Figure 10: Final result</p>
</div>
...
<img src="../../../../../feature/archive/202606/0/image/10x10网格1000次尝试成功生成次数曲线图.png" alt="" style="zoom:20%;" /><p style="color: gray;">Figure 11: Curve chart of the number of successful generation times for 10x10 grid after 1000 attempts</p>
</div>
...
<img src="../../../../../feature/archive/202606/0/image/不同尺寸网格生成接水管耗时箱线图（单位：ms）.png" alt="" style="zoom:20%;" /><p style="color: gray;">Figure 12: Box plot of time consuming for generating water connection pipes with grids of different sizes (unit: ms)</p>
</div>
...
<img src="../../../../../feature/archive/202606/0/image/10x10网格接水管解题判定耗时箱线图（单位：ms）.png" alt="" style="zoom:20%;" /><p style="color: gray;">Figure 13: Box plot of time-consuming determination of 10 x 10 grid water pipe problem solving (unit: ms)</p>
</div>
