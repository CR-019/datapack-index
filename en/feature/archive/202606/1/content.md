...
- `visiting`: The path DFS is currently taking, the corresponding block is a yellow frog light.`ochre_froglight`。
    - `conflict_n`: The number of times a location conflicts, that is, the number of attempts to access the location after having already passed the location once. This item is not recorded as a block but in the display box entity.`pipes_conflict`In the scoreboard, that is, it can exist simultaneously with the previous items.
...
The recursive backtracking process of DFS is the opposite of the search path, so when backtracking,`conflict`The place passed before the state is a loop, and the place passed after encountering it (that is, the area traveled before the loop was searched) is not part of the loop.
...
2. If the point is already at`visiting`state:
    - to all in`visiting`State point (if it is a non-entity solution, active backtracking is required here) Add`ring`status, becomes`visiting_ring`state.
    - Add to this point`conflict`status, becomes`visiting_conflict_ring`state.
    - return.
3. Search in all non-source directions.
4. When backtracking, if the point has`conflict`state:
    - to all at the same time`visiting_ring`status point (excluding itself), clear it`ring`state.

In this way, we can mark the rings on the picture. The pseudocode is as follows:
```python
...
Specifically, put`ring`The status is changed to a numerical value. If a conflict is encountered, all nodes being visited will`ring`Add 1 to the value and subtract 1 if there is a backtracking conflict. The pseudocode is as follows:
```python
function dfs(now_tile,from_direction)
...
But since we know that they cover each other, the solution is also obvious, which is to put`conflict`The status is also changed to numerical record. The final pseudocode is as follows.
```python
function dfs(now_tile,from_direction)
    if now_tile.RING >= 1 do
        return
...
### 2.4.1 Choice of partial inspections
...
The site range is$31*3*31$range, where the third layer stores whether each cell has been accessed, and the`1~2`The layer stores the connecting edges in the x-axis direction and z-axis direction respectively.
...
- Only candidate edges (directed edges) are marked with entity. In the generation phase, all marking entities that mark candidate edges are always active in the fourth layer (that is, all block surfaces), and block mark access is placed in the third layer.
    - The entity marking a candidate edge is always located at the start point of the edge, facing the end point of the edge. All such entities contain tag`pipes_vec`and`pipes_（方向）` 。
...
- The first and second layers store the edges (undirected edges) connecting the grids, forming a "block map". Generation and verification mainly rely on the "block map", but the disruption and visual interaction are handled by the "entity map".
    ...
    - Afterwards, each time an operation is performed on a grid, only the edges connected to the grid in the "block map" will be updated.
...
- In addition to the map display frame of the "Water Source" grid on the map, it has`pipes_source`tag also owns`pipes_checkerboard` tag。
    - That is to say, the display frame entity is the starting entity for all behaviors of the chessboard including generation and testing. See below for details.
...
#### 3.2b.2 Scramble and re-record related functions
...
This part of the function has nothing to do with the algorithm, but is only related to material preparation.
...
and function`pipes:validate/dfs/wet/（东南西北四个方向）`
```mcfunction
...
function `pipes:validate/dfs/dry/（方向）`
```mcfunction
# 不用标环路，直接返回。
...
We need to use the map website to create the following 18 materials and export them to dat format.`存档文件夹/data/minecraft/maps`Replace the ID with`0~19`(with jump numbers) map. (You may need to draw 20 maps first.)

![alt text](../../../../../feature/archive/202606/1/2026-06-04_02.08.54.png)

Among them, map`0~3`It's dry,`4~7`It's moist,`9~11`It is a cycle (obviously a grid that only connects one edge cannot be part of a cycle, so 8 is skipped);`12~15`It's a water source,`17~19`It's the water source loop. The map material numbers are in the following order.
...
- Map 0, a point. (The one made here is downward)
