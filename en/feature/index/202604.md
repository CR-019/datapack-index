---
title: '"Feature" 2026.04'
outline: Outline[1]
---
<!-- markdownlint-disable MD033 MD041 -->
<script setup>
    import { useData } from 'vitepress'

    const { isDark } = useData()
</script>

<JournalHead
    cover = "../../../feature/cover/202604/202604.png"
    :editors = "['CR_019','Alumopper']"
    :credits="['徐木弦']"
    :launchLink="{ link: '../archive/202604/ifc/content', text: 'Second cover' }"
    :coverLink="'https://www.bilibili.com/video/BV1NEojBeERk'"
/>

# Featured
<IndexCompatible
    title = "Have you ever played infinite randomly generated parkour maps? &quot;Leaping into the Crystal World 2&quot;"
    url = "../archive/202604/0/content"
    authorName = "Xu Muxian"
    abstract = "When we were kids, there was a game on iOS that embraced us: Smash hit. Now, we've ported its ideas to Minecraft's parkour maps. In this map, we have limited the number of jumps the player can make. They can only increase the number of jumps through supply crystals in the game. If you fall, 10 jump points will be deducted. The game ends when the number of jumps reaches zero."
    background = '../../../feature/archive/202604/_assets/0.jpg'
/>

# Vanilla News Λojang Spotlight
<IndexCompatible
    title = "Vanilla News - April 2026"
    url = "../archive/202604/spotlight/content"
    authorName = "Alumopper"
    abstract = "This month Mojang released the official version of 26.1, and also released three snapshots 26.2-snapshot-1~3 belonging to 26.2. Currently, the data packversion has reached 102.0 and the resource packversion has reached 86.0. This month’s update adds Vulkan support and has introduced content related to sulfur monsters. This month’s update has medium practicality and medium destructiveness, and is generally below the level of a big cup."
    background = '../../../feature/archive/202604/_assets/spotlight.png'
/>

# Insights Insights

<IndexCompatible
    title = "Lightweight and low-loss algorithm and program implementation for converting OBJ model to voxel model"
    url = "../archive/202604/1/content"
    authorName = "Xuanyu1725"
    abstract = "The OBJ format model is very commonly used in the industry, but to convert it into a model usable in Minecraft, it needs to be converted into voxels. Most of the OBJ conversion models on the market simply discretize the OBJ model into a grid with fixed side lengths. This method is poor in performance, art style and maintainability. This article will discuss the program implementation and optimization strategies of this framework based on the basic framework mentioned by the author in the 25.12 monthly submission. During the implementation process, We will discuss in depth the application of mathematical tools such as graph theory, number theory, and linear algebra in the process of model voxelization."
/>


<IndexCompatible
    title = "Use PackSquash to compress and obfuscate resource packs"
    url = "../archive/202604/2/content"
    authorName = "bread"
    abstract = "PackSquash is a resource pack compression and obfuscation tool that can help greatly reduce the size of resource packs and perform simple obfuscation. The use of the tool is not complicated, but after all, it is a command line tool, which is still a bit troublesome for those who are not familiar with it. This article will briefly introduce how to use this powerful tool."
/>


<IndexCompatible
    title = "Development experience sharing: entity tree and object-oriented"
    url = "../archive/202604/3/content"
    authorName = "Xuanyu1725"
    abstract = "This article refers to the two articles &quot;You Must Leave All World Entities Immediately to Use the New Generation of Shadow Operators - Rainbow&quot; and &quot;Minecraft Instruction Cooking Guide: Cold Entity Selector - Chuang Xiaoye&quot; included in the site, which share an entity management and operation method based on a similar tree structure."
/>


<IndexCompatible
    title = "Command Storage Advanced: Using Stack to Manage Function Context"
    url = "../archive/202604/4/content"
    authorName = "Qi Bai"
    :extraAuthors="['Xu Muxian']"
    abstract = "This article discusses how to optimize the data management and logic design of Minecraft function (mcfunction, hereafter referred to as mcf) by simulating the stack structure. In view of the pain points of native mcf scope limitations, process data residues and inefficient cross-function communication, variable isolation and context transfer are achieved through manual management of the &quot;push/pop&quot; mechanism, and recursive calls and dynamic scope nesting are supported. This solution uses the command storage system (storage) to replace macro parsing, improve performance and reduce resource redundancy, and provide complex data Pack development provides clear scope control and maintainability, especially in scenarios such as multi-layer nesting and algorithm implementation, which significantly enhances logic clarity and execution efficiency, and provides a standardized development model for mcf. "
/>

<IndexCompatible
    title = "Floating UI practice - custom controls"
    url = "../archive/202604/5/content"
    authorName = "Alumopper"
    abstract = "This issue is the second practical tutorial on the Floating UI front-end library. It introduces how to define and use custom controls, such as sliders, etc."
    background = '../../../feature/archive/202604/_assets/5.png'
/>

# craftsman Masterpieces
<IndexCompatible
    title = "My worldvanilla video player"
    url = "../archive/202604/6/content"
    authorName = "Luo Fenglan_Sea"
    abstract = "This project is based on the equipment mask feature and provides a sequence image playback in vanillaMinecraft to achieve the effect of pseudo video playback. Can be used to insert pre-made cg etc. into the map."
    background = '../../../feature/archive/202604/_assets/6.png'
/>

<ClientOnly>
  <GiscusComment
    repo="CR-019/datapack-index"
    repoId="R_kgDONRhuqw"
    category="Chats"
    categoryId="DIC_kwDONRhuq84CkchW"
    mapping="number"
    term="64"
    :strict="false"
    :reactionsEnabled="true"
    emitMetadata="0"
    inputPosition="top"
    :theme="isDark ? 'dark' : 'light'"
    lang="zh-CN"
    loading="lazy"
    class="giscus-wrapper"
  />
</ClientOnly>

<style>
.giscus-wrapper {
  margin: 3rem auto;
  max-width: 800px;
  padding-top: 2rem;
  border-top: 1px solid var(--vp-c-divider);
}
</style>