...
1. The player holds a specific nbt item
2. The player’s scoreboard has counts.
...
- After all, the display entity is not like the item, it is another entity independent of the player. Therefore, it is more suitable to be used as an NPC or button, fixed at a specific location on the map.
  ...
  - But this will cause a problem, that is, if the player can also perform other operations that can use left and right keys, the operation of the interactive entity will be triggered first, so that other operations cannot be triggered;
  - Therefore, this method is suitable for highly customized maps that rewrite all various operations and use them as the triggering interface for left and right key events.
- The right-click behavior of the interactive entity is consistent with the carrot fishing rod, which is triggered once every 4 ticks; the left-click behavior is consistent with the interactive behavior of attacking other mobs, which is triggered once with a single click, and has no effect if long-pressed;
  - Therefore, the past methods of detecting left clicks, such as spawning villagers on the player's face, can be replaced.
...
### Other discussions
- When this component was not available in the past, we would use the Eye of Ender for right-click detection.
...
- Actually, by setting`consume_seconds`We can also use this component to detect long presses. We'll get to that in a minute.
