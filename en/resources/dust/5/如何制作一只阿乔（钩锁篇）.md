...
Aim for a ready-made solution. I actually used this method in the third issue ["Commonly used right-click detection"](/en/resources/dust/3/常用右键检测.md) has been explained.
...
- How to create an explosion?
  - There are several alternatives: creepers, tnt (entity), and ender crystals, all of which can provide instant explosions;
- How to prevent explosions from destroying the terrain and damaging surrounding entities?
  ...
  - So, we determined the entity to use: ender crystal. Just give it a little damage and it will explode immediately.
- How to prevent the player from getting hurt?
  - Switch to creative mode before exploding, then switch back.
  - Therefore, a scoreboard needs to be established to record the current game mode.
...
Therefore we need a predicate to distinguish whether the player is on the ground:
...
However, the actual effect is that the player will fall slowly, but it does not affect the aiming very much, so I will make do with it.
...
- After launching from the ground, land;
...
- A method that is easier and more effective is to only determine the cooling conditions when the player is holding Aqiao in his hand, so that it does not affect the use. However, it is necessary to ensure that the tag marking Aqiao's status is synchronized with the item, or to directly detect the item data.
