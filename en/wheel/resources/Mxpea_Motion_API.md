...
Add the following Tag tag to the entity to throw the entity in the direction you are facing:`mot`: Launch the entity to your line of sight;`random_mot`，`random_mot_mid`，`random_mot_big`: Add randomness to the emission. These three are different presets. Of course, you can also customize the randomness in the data pack file;`random_exp`: Launch entities in random directions. Applying to multiple entities at once can make them look like explosions;`trace`: Launch entity to the nearest player;`away`: Emit entities in a direction away from the nearest player;`one_random_mot`: Emit entities in random directions, but it cannot provide different randomness for multiple entities at the same time.